...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance(); //Inicializa la libreria de vulkan, es la conexion entre la aplicacion y la api
  /*Establecemos capas de validacion, Vulkan es muy conservador con el checking de errores en runtime, por tanto muchos de ellos no se capturan
  y pueden acabar en un crash o comportamiento indefinido, lo activaremos unicamente en development para debugging y lo desactivaremos para build de release 
  para aprovechar el rendimiento al maximo*/
  setupDebugMessenger(); 
  createSurface(); //Creamos la superficie, depende de glfw y es la conexion entre la ventana y la habilidad de Vulkan para mostrar elementos
  pickPhysicalDevice(); //El physical device es el dispositivo que utilizaremos para correr el programa, en este caso la GPU 
  createLogicalDevice(); //Elegimos con esto las features que queremos utilizar de las que disponemos 
  createCommandPool(); //Ayudara con la alocacion del buffer de comandos
}
